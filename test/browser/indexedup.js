...
require.define("/indexedup.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  var ERRORS, IDBKeyRange, IDBTransaction, IUDatabase, IndexedUp, ReadableStream, indexedDB, prefixed, stream,
  ...
  ERRORS = {
    not_found: function(key) {
      return new Error("Key not found in database [" + key + "]");
    },
    not_open: function() {
  ...
  IUDatabase = (function() {
    ...
    function IUDatabase(_location, _options) {
    ...
    IUDatabase.prototype.open = function(cb) {
      ...
      request.onerror = function(e) {
        return typeof cb === "function" ? cb(new Error("Failed to open IndexedDB (@_location)", e)) : void 0;
      };
    ...
    IUDatabase.prototype.getStore = function(write) {
    ...
    IUDatabase.prototype.put = function(key, data, cb) {
      var req;
      if (!this.open) {
        return cb(ERRORS.not_open());
      }
      if (!((key != null) && (data != null))) {
        return cb(ERRORS.invalid_input());
      }
      req = this.getStore(true).add({
        key: key,
        value: data
      });
      req.onsuccess = function(e) {
    ...
    IUDatabase.prototype.get = function(key, cb) {
      var req;
      if (!this.open) {
        return cb(ERRORS.not_open());
      }
      ...
      req.onsuccess = function(e) {
        var result, _ref;
        if (result = (_ref = req.result) != null ? _ref.value : void 0) {
          return cb(null, result);
        } else {
          return cb(new Error(ERRORS.not_found(key)));
        }
      };
      return req.onerror = function(err) {
        return cb(err);
      };
    };

    IUDatabase.prototype.del = function(key, cb) {
      var req;
      if (!this.open) {
        return cb(ERRORS.not_open());
      }
      ...
      return req.onerror = function(e) {
        return cb(ERRORS.not_found(key));
      };
    };
